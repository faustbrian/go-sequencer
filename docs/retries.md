...
supplies `Attempt.Budget`; `adapters/retry.Adapter` consumes that shared budget
before every callback. It cannot exceed the declared maximum even if an external
